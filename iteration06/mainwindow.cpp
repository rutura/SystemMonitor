    ...
    , m_systemMonitor(nullptr)
{
    ...
    // Create process table as central widget
    m_processTable = new ProcessTableWidget(this);
    setCentralWidget(m_processTable);
    
    // Apply theme to the process table
    m_processTable->updateTheme();
...
void MainWindow::setupSystemMonitor()
{
    ...
    // Connect signal to update process table
    connect(m_systemMonitor, &SystemMonitor::dataUpdated,
...
{
    if (m_processTable && m_systemMonitor) {
        // Get top 10 processes
        auto topProcesses = m_systemMonitor->getTopProcesses(10);
        m_processTable->updateProcessList(topProcesses);
    }
}
